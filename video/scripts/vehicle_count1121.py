...
class VehicleCounter():
    ...
    def start(self):
        ...
        while True:
            ...
            cv2.line(img, (self.oxcoord,self.oycoord), (self.oxcoord,self.dycoord), (187, 0, 255), 2)
            cv2.line(img, (self.oxcoord,self.dycoord), (self.dxcoord,self.dycoord), (187, 0, 255), 2)
            cv2.line(img, (self.dxcoord,self.dycoord), (self.dxcoord,self.oycoord), (187, 0, 255), 2)
            cv2.line(img, (self.dxcoord,self.oycoord), (self.oxcoord,self.oycoord), (187, 0, 255), 2)
    ...
    def count_vehicle(self,box_id, img):
        ...
        if (liney > iy) and (ix > self.oxcoord) and (ix < self.dxcoord) and ( iy > self.oycoord ) and  (iy < self.dycoord ) :
           ...
           print('top triangle here', ix,' ', iy , 'liney is', liney)         
           if id not in self.temp_up_list:
            ...
            print('bottom triangle here', ix,' ', iy , 'liney is', liney)
            if id not in self.temp_down_list:
                self.temp_down_list.append(id)

        elif (iy < self.oycoord) and (iy < self.dycoord) :
